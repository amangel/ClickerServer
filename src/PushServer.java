...
public class PushServer {
    ...
    private static String GRAVE_SEPARATOR       = "`/`";
    private static String AMPERSAND_SEPARATOR   = "`/&";
    private static String AT_SEPARATOR          = "`/@";
    ...
    private void readQuestionSet(String setString) {
        ...
        String[] setParts = setString.split(AT_SEPARATOR);
        System.out.println(setParts[0]);
        ArrayList<String> questionList = new ArrayList<String>();
        if (setParts.length > 1) {
            if (!setParts[1].equals("")) {
                String[] setQuestions = setParts[1].split(AMPERSAND_SEPARATOR);
    ...
    public String getQuestionSets() {
        ...
        while (qSetIter.hasNext()) {
            ...
            i++;
            //finalResult = finalResult + qSetIter.next() + AMPERSAND_SEPARATOR;
        }
    ...
    public String getQuestionsInSet(String qSetKey) {
        ...
        for (int i=0; i<setQs.size(); i++) {
            ...
            finalResult = finalResult + next.getID() + SEMI_COLON_SEPARATOR + next.getFlags() + SEMI_COLON_SEPARATOR 
                    + next.getWidgets() + SEMI_COLON_SEPARATOR + next.getColor() + AMPERSAND_SEPARATOR;
        }
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    String[] idparts = idString.split(SEMI_COLON_SEPARATOR);
                    if (idparts.length == 3) {
                        String cid = idparts[0];
                        String cmac = idparts[1];
                        String[] adminParts = idparts[2].split(COMMA_SEPARATOR);
                     ...
                     try {
                        ...
                        out.println("Open" +SEMI_COLON_SEPARATOR + idparts[3] + SEMI_COLON_SEPARATOR + requested.getFlags() + SEMI_COLON_SEPARATOR 
                                + requested.getWidgets() + SEMI_COLON_SEPARATOR + requested.getColor());
                    } 
                     ...
                     try {
                        ...
                        if (previous.hasFollowUp()) {
                            ...
                            out.println("Open" +SEMI_COLON_SEPARATOR + next.getID() + SEMI_COLON_SEPARATOR + next.getFlags() + SEMI_COLON_SEPARATOR 
                                    + next.getWidgets() + SEMI_COLON_SEPARATOR + next.getColor());
    ...
    private class AdminListener implements Runnable {
        ...
        public void run() {
            while (true) {
                ...
                try {
                    System.out.println("Waiting for admin on push server "+System.currentTimeMillis());
                    adminSocket = connectionSocket.accept();
                    adminSocket.setKeepAlive(true);
                    System.out.println("Admin connected, authenticating now "+System.currentTimeMillis());
