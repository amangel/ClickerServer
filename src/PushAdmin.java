...
public class PushAdmin {
	...
	public void adminReconnected(Socket newSocket, BufferedReader newIn, PrintWriter newOut) {
		...
		for (int i=0; i<displays.size(); i++) {
			out.println("DisplayConnected`/`" + displays.get(i).getIConsume());
		}
		commandHandler = new CommandHandler();
		timer = new Timer();
		new Thread(commandHandler).start();
	...
	public void processDisplay(Socket display, BufferedReader displayIn, PrintWriter displayOut) {
		...
		out.println("DisplayConnected`/`" + newDisplay.getIConsume());
	}
	...
	/*
	private class DisconnectionListener implements Runnable{
		...
		public void run() {
			while (true) {
				try {
					...
					while (iter.hasNext()) {
						Map.Entry<String, Client> next = iter.next();
						String cKey = next.getKey();
						Client cconn = next.getValue();
						try {
							if (cconn.getReader().readLine() == null) {
								cconn.getSocket().close();
								groups.get(clients.get(cKey).getGroup()).remove(cKey);
								clients.remove(cKey);
								System.out.println("Client disconnected with id: " + cKey);
								Iterator<String> groupIter = groups.keySet().iterator();
								String groupUpdate = "GroupList`/`";
								while (groupIter.hasNext()) {
									groupUpdate += groupIter.next() + "`/;";
								...
								out.println(groupUpdate);
								//out.println("ClientDisconnected;" + cKey);
							}
						} catch (IOException e) {}
					}
	...
	*/
	...
	private class CommandHandler implements Runnable{
		...
		public void unpause() {
			paused = false;
			System.out.println("Admin just got unpaused");
		}
		
		public void pause() {
			paused = true;
			timer.cancel();
			System.out.println("Just paused admin");
		}
